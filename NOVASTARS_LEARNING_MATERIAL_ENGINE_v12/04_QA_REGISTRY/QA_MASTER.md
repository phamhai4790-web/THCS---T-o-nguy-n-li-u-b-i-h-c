...
> **Role**: Hệ thống Thẩm định Master & Bộ Kiểm định Kiến thức Architecture QA (AQ1 – AQ13)  
...
Hệ thống QA Registry quản lý toàn bộ **42 Lesson QA Tests** cùng **13 Architecture QA Tests** phân chia thành các sub-suite độc lập:

```text
04_QA_REGISTRY/
├── QA_MASTER.md                       <-- [File hiện tại: QA Workflow, AQ1..AQ13, M1..M8]
...
├── KNOWLEDGE_SKILL_PRODUCT_QA.md      <-- [KC1..KC3, SK1..SK2, PD1..PD2, R41 No-Spoil Audit]
└── TRACEABILITY_QA.md                 <-- [C7 Matrix Audit & C8.1..C8.5 Skill Necessity Tests]
```

---

## 2. BỘ ARCHITECTURE QA TESTS (AQ1 — AQ13)

Đây là 13 bài kiểm tra kiến thức hệ thống v12.0, bắt buộc phải đạt `PASS 13/13` trước khi phát hành phiên bản:
...
| **AQ2** | No Duplicated Global Rule | Core vs Roles | 41 Master Rules chỉ nằm ở `01_CORE/GLOBAL_RULES.md`. 0% trùng lặp. |
| **AQ3** | No Orphan Rule | Bảo toàn Quy tắc | Tất cả 41 Rules v11 trace 100% vào `GLOBAL_RULES.md`. |
| **AQ4** | No Orphan Template | Registry Schemas | Tất cả Dạng Template liên kết trực tiếp với Role Family. |
| **AQ5** | No Orphan QA Test | QA Registry | Tất cả 42 QA tests nằm ở `04_QA_REGISTRY/`. 0% thiếu hụt. |
...
| **AQ13** | Constructivist No-Spoil Engine| Constructivist Engine | Part A 0% Spoil tên công cụ/bẫy tư duy; Nhiệm vụ tuân thủ chuỗi 3 Tầng. |

---

## 3. MISSION NATURALNESS & NO-SPOIL AUDIT SUITE (M1 — M8)

8 bài kiểm tra văn phong nhập vai tự nhiên & không spoil đáp án của Student Mission:

1. **TEST M1 — ROLE VOICE AUTHENTICITY**: Văn phong nhập vai tự nhiên theo đúng Role Spec. (PASS/FAIL)
2. **TEST M2 — QUESTION-DRIVEN DISCOVERY**: Nhiệm vụ dẫn dắt bởi câu hỏi tự nhiên theo 3 Tầng Khám phá. (PASS/FAIL)
3. **TEST M3 — PROPOSED ACTION FRAMING**: Mission yêu cầu học sinh ĐỀ XUẤT giải pháp ($C_4$). (PASS/FAIL)
4. **TEST M4 — NO PRE-BAKED DISCOVERY (NO-SPOIL)**: Part A 0% Spoil tên công cụ, bẫy tư duy, tên kết luận hay quy trình trước khi học sinh khám phá. (PASS/FAIL)
5. **TEST M5 — AGE APPROPRIATENESS**: Ngôn ngữ phù hợp khối học sinh Khối 8/9. (PASS/FAIL)
6. **TEST M6 — PURE VIETNAMESE**: 100% Tiếng Việt thuần khiết, 0% từ Tiếng Anh trong Part A & B. (PASS/FAIL)
7. **TEST M7 — DELIVERABLE CLARITY**: Sản phẩm cần nộp là kết quả tự nhiên của quá trình giải quyết vấn đề. (PASS/FAIL)
